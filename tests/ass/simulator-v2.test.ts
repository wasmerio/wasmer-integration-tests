// v2 reconciler: the declaration half and the planner are pure, so every
...
import {
  ...
  key,
  type Resource,
} from "../../ass/simulator/v2/model";
import { existsSync, mkdtempSync, writeFileSync } from "node:fs";
import os from "node:os";
import path from "node:path";
import { dump as dumpYaml } from "js-yaml";
import { digestFingerprint } from "../../ass/simulator/v2/digest";
import { runDiffV2, runDownV2 } from "../../ass/simulator/v2/verbs";
import { ESC } from "../../ass/report/style";
import {
  duration,
  planRows,
  ReconcileReporter,
} from "../../ass/simulator/v2/render";
import {
  ledgerPath,
  writeLedger,
  type LedgerFile,
} from "../../ass/simulator/v2/ledger";
import { IdentityMap } from "../../ass/simulator/v2/identity";
import { planReconcile } from "../../ass/simulator/v2/plan";
import { defaultDiff, type DiffContext } from "../../ass/simulator/v2/adapter";
import { diffBucket } from "../../ass/simulator/v2/adapters/requests";
import {
  buildWorld,
  expandKind,
  summarizeExpansion,
  ExpansionError,
} from "../../ass/simulator/v2/expand";
import {
  validateDeclaration,
  upgradeV1,
  parseOffsetMs,
} from "../../ass/simulator/v2/scenario";
import {
  resolveWorkers,
  Semaphore,
  mapConcurrent,
} from "../../ass/simulator/v2/engine/context";
import {
  stageKinds,
  assertMutationStats,
} from "../../ass/simulator/v2/engine/scheduler";
import type { RequestBucketSpec } from "../../ass/simulator/v2/specs";
...
function world(overrides: Record<string, unknown> = {}) {
  return buildWorld({
    declaration: validateDeclaration(declaration(overrides), "unit.yaml"),
    seed: 4242,
    anchorMs: ANCHOR,
  });
}
...
describe("expansion (declaration half)", () => {
  ...
  it("upgrades a v1 declaration onto the v2 shape", () => {
    ...
    const parsed = validateDeclaration(upgraded, "legacy.yaml");
...
describe("verbs against a platform that is not running", () => {
  ...
  function heldLedger(root: string): LedgerFile {
    const declared = validateDeclaration(declaration(), "unit.yaml");
    return {
      stateVersion: 2,
      engine: "v2",
      slug: "unit",
      mode: "up",
      assSchema: 2,
      scenarioPath: path.join(root, "unit.yaml"),
      seed: 4242,
      declarationDigest: "d",
  ...
  it("down releases the hold instead of booting a platform", async () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "sim-v2-down-"));
    writeLedger(root, heldLedger(root));
    const fake = fakeDriver(root, false);
    const out: string[] = [];
    const code = await runDownV2({
      slug: "unit",
      cwd: root,
  ...
  it("diff prints the expansion instead of booting a platform", async () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "sim-v2-diff-"));
    writeFileSync(path.join(root, "unit.yaml"), dumpYaml(declaration()));
    const fake = fakeDriver(root, false);
    const out: string[] = [];
    const code = await runDiffV2({
      file: "unit.yaml",
      cwd: root,
      io: { out: (line) => out.push(line), err: () => undefined },
      deps: { driver: fake.driver as never },
  ...
  it("verify fails when there is no platform to verify against", async () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "sim-v2-verify-"));
    writeFileSync(path.join(root, "unit.yaml"), dumpYaml(declaration()));
    const fake = fakeDriver(root, false);
    const code = await runDiffV2({
      file: "unit.yaml",
