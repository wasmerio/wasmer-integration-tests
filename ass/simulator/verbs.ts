// The lifecycle verbs: `up` (reconcile to the declaration), `diff` (plan
// only), `verify` (plan must be empty), `down` (reconcile to the empty
// set). All four are the same loop; only the desired set and the exactness
...
import { parseToml } from "../scenario/toml";
import { assertLocalOnly, GuardRefusalError } from "./guard";
import { platformIsLive, resolvePlatform } from "./platform";
import { acquireHeldLock, HeldLockError } from "./lock";
import { resolveSeed, seedLine } from "./random";
import { loadDeclarationFile, SimulatorLoadError } from "./scenario";
...
export interface VerbOptions {
  ...
  color?: boolean;
  /** Test seam: fake the platform driver / liveness probe / adapters. */
  deps?: SimulatorDeps;
}
...
/** The store seam: injected fake adapters win over the built-in registry. */
async function adaptersFor(
  deps: SimulatorDeps | undefined,
): Promise<Map<ResourceKind, ResourceAdapter>> {
  if (deps?.adapters !== undefined) {
    return new Map(deps.adapters.map((adapter) => [adapter.kind, adapter]));
  }
...
export async function runUp(options: VerbOptions): Promise<number> {
  const { io } = options;
  ...
  try {
    ...
    try {
      ledger = readLedger(repoDir, declaration.name);
    } catch (err) {
    ...
    try {
      const adapters = await adaptersFor(options.deps);
...
/** A `--file` argument only locates the slug; the ledger is the truth. */
function slugFromFile(
  ...
  let name: unknown;
  try {
    const data = parseToml(raw, filePath);
...
export async function runDown(options: VerbOptions): Promise<number> {
  const { io } = options;
  const driver = driverFor(options);
  const repoDir = driver.repoDir;
  const reporter = reporterFor(options);
  ...
  if (options.file !== undefined) {
    const located = slugFromFile(options.cwd, options.file, io);
    if (located === null) {
      return EXIT_USAGE;
    }
    slug = located;
    ...
    if (listing.ledgers.length === 0) {
      io.out("nothing is held — no descriptor under .ass/state/");
      return EXIT_OK;
    }
    if (listing.ledgers.length > 1) {
      io.err(
        "error: multiple scenarios are held; name one: " +
          listing.ledgers.map((ledger) => ledger.slug).join(", "),
      );
      return EXIT_USAGE;
    }
    slug = listing.ledgers[0].slug;
  }

  let ledger: LedgerFile | null;
  try {
    ledger = readLedger(repoDir, slug);
  } catch (err) {
    if (err instanceof CorruptStateError) {
      io.err(`error: ${err.message}`);
      return EXIT_USAGE;
  ...
  // An edited file must not stop its own cleanup: the ledger records the
  // last declared intent, and the teardown reconciles from that.
  if (
    fileRaw !== null &&
    digestDeclaration(fileRaw) !== ledger.declarationDigest
  ) {
    io.err(
      `warning: ${options.file} has changed since "${slug}" was seeded; ` +
        "proceeding from the ledger (it records what was declared)",
    );
  }
