...
// `--plan`, the writer, and the descriptor's recorded projections, so what
// the plan prints, what lands in ClickHouse, and what Phase 6 asserts are
// the same numbers by construction. Hour counts are keyed by absolute epoch
// hour (fork label), so reseeding with the same seed on the same day
// reproduces exact per-day totals.

import { parseDurationMs, parseSizeBytes } from "./schema";
import type { TelemetryBlock } from "./schema";
