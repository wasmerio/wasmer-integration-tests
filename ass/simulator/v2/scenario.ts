// The v2 declaration: what a human writes. Purely declarative - it names
// no store, no SQL and no SDK call, which is what makes the declaration
// half unit-testable with zero infrastructure.
//
// v1 files (`assSchema: 1`) load unchanged: `upgradeV1` maps them onto the
// v2 shape, so every committed scenario reconciles without an edit and
// Q-B's "rebuild once" is the only migration a world ever needs.

import { readFileSync } from "node:fs";
import { load as parseYaml, YAMLException } from "js-yaml";
import { z, ZodError } from "zod";
import { FIXTURE_NAMES } from "../fixtures";
import { formatIssues, SimulatorLoadError } from "../schema";
...
const accountSchema = z.strictObject({
  ...
  /** The superuser lens; v2 spells it `policy.prune: "retain"`. */
...
/** v1 -> v2. The only shape change is precision: v1's `rawWindow` becomes
 * `precision.raw`, and everything else is a superset. */
  ...
  if (!("assSchema" in record)) {
    throw new SimulatorLoadError(
      `${sourcePath}: no assSchema key - this is not a simulator declaration. ` +
        `Simulator files declare \`assSchema: ${SUPPORTED_SCHEMA}\` (v1 files ` +
        "with `assSchema: 1` are upgraded automatically); ASS reproduction " +
