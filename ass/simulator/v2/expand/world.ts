...
export interface World {
  ...
  /** v1's proven traffic expansion, anchored at `anchorMs`. */
...
function fixtureAssignments(
  ...
  // Same seeded shuffle as v1, so a v1 world and a v2 reconcile of the same
  // seed agree on which app runs which fixture (invariant I5).
...
export function buildWorld(input: {
  ...
  // The same fork label v1 uses, so a given seed names the same portfolio
  // under both engines - which is what makes invariant I5 checkable.
  ...
  // v1's weight stream, reproduced exactly: the traffic model owns it, and
  // the world mirrors it so per-app shares are stable across engines.
  ...
  if (telemetry !== undefined && count > 0) {
    ...
    traffic = expandTraffic(
      { ...telemetry, rawWindow: telemetry.precision.raw } as never,
      seed,
      count,
      anchorMs,
      multipliers,
